...
export function requestPdfDownload(
  type: "original" | "response" = "original",
): void {
  window.dispatchEvent(
    new CustomEvent("sudox:download-pdf", { detail: { type } }),
  );
}

/** Listen for PDF download requests. Returns unsubscribe fn. */
export function onPdfDownloadRequest(
  handler: (type: "original" | "response") => void,
): () => void {
...
function _drawGrid(
  ...
  const { gridDark, clueText, altCell, diagCell, winCell, whitish } =
    PDF_PALETTE;
  ...
  doc.setFont("Helvetica", "normal");
  doc.setFontSize(14);
  doc.text("https://sudox-app.vercel.app/", cx + 10, cy1 + 10, {
    align: "center",
    angle: 45,
  });
  ...
  doc.setFont("Helvetica", "normal");
  doc.setFontSize(14);
  doc.text("https://sudox-app.vercel.app/", cx + 10, cy2 + 10, {
    align: "center",
    angle: 45,
  });
  ...
  const sortedLines = Array.from(linesToDraw.values()).sort(
    (a, b) => a.w - b.w,
  );
...
function _drawFooter(doc: JsPDFDoc) {
  ...
  doc.textWithLink(text1, startX, footerY, {
    url: "https://www.linkedin.com/in/rathodnk",
  });
